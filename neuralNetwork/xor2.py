...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan  7 15:06:47 2020
...
class NN:
    ...
    def __init__(self, input_layer, hidden_layer, output_layer):
        #initaite weights
        self.w_hidden_layer = np.random.random((input_layer, hidden_layer)) 
        self.w_output_layer = np.random.random((hidden_layer, output_layer)) 
        #bais per layer
        self.hidden_layer_bias = np.random.random((1,hidden_layer)) 
        self.ouput_layer_bais = np.random.random((1,output_layer)) 
...
hidden_out, output_out = nn.predict(inputs)
print(output_out)
